...
0. [Obtain a current source tree](#0-obtain-a-current-source-tree)
1. [diff -up](#1-diff--up)
...
5. [Select the recipients for your patch](#5-select-the-recipients-for-your-patch)
...
7. [E-mail size](#7-e-mail-size)
8. [Respond to review comments](#8-respond-to-review-comments)
9. [Don't get discouraged - or impatient](#9-dont-get-discouraged---or-impatient)
10. [Include PATCH in the subject](#10-include-patch-in-the-subject)
11. [Sign your work — the Developer's Certificate of Origin](#11-sign-your-work--the-developers-certificate-of-origin)
12. [When to use Acked-by: and Cc:](#12-when-to-use-acked-by-and-cc)
...
15. [Explicit In-Reply-To headers](#15-explicit-in-reply-to-headers)
16. [Sending git pull requests](#16-sending-git-pull-requests)
...
Make sure your patch does not include any extra files which do not belong in a patch submission. Make sure to review your patch _after_ generating it with `diff(1)`, to ensure accuracy.

If your changes produce a lot of deltas, you need to split them into individual patches which modify things in logical stages; see [Section 3: Separate your changes](#3-separate-your-changes). This will facilitate review by other kernel developers, very important if you want your patch accepted.
...
The maintainer will thank you if you write your patch description in a form which can be easily pulled into Linux's source code management system, git, as a "commit log". See [Section 15: Explicit In-Reply-To headers](#15-explicit-in-reply-to-headers).

**Solve only one problem per patch.** If your description starts to get long, that's a sign that you probably need to split up your patch. See [Section 3: Separate your changes](#3-separate-your-changes).
...
Linus Torvalds is the final arbiter of all changes accepted into the Linux kernel. His e-mail address is <torvalds@linux-foundation.org>. He gets a lot of e-mail, and, at this point, very few patches go through Linus directly, so typically you should do your best to _avoid_ sending him e-mail.
...
1. A `from` line specifying the patch author (only needed if the person sending the patch is not the author).
2. An empty line.
3. The body of the explanation, line wrapped at 75 columns, which will be copied to the permanent changelog to describe this patch.
4. The `Signed-off-by:` lines, described above, which will also go in the changelog.
...
_© Copyright 2016, The kernel development community._
